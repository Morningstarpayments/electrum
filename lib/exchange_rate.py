...
class Bleutrade(ExchangeBase):
    def get_rates(self, ccy):
        json = self.get_json('bleutrade.com/api/v2', '/public/getticker?market=MRNG_%s' % ccy)
        # {'BTC': Decimal(0.1234)}
    ...
    def historical_rates(self, ccy):
        history = self.get_json('bleutrade.com/api/v2',
                               "/public/getmarkethistory?market=MRNG_%s&count=100" % ccy)
...
class Cryptopia(ExchangeBase):
    def get_rates(self, ccy):
        json = self.get_json('www.cryptopia.co.nz', '/api/GetMarket/MRNG_%s' % ccy)
...
class C_Cex(ExchangeBase):
    ...
    def historical_rates(self, ccy):
        history = self.get_json('c-cex.com',
                               "/t/api_pub.html?a=getmarkethistory&market=MRNG-%s&count=10" % ccy)
        return dict([(h['TimeStamp'][:10], h['Price'])
                     for h in history['result']])
